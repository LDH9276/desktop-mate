...
export function WindowSizeSettings() {
  ...
  return <fieldset className="window-size-settings"><legend>창 크기와 위치 <span><Pin size={10} />항상 위에 표시</span></legend>
    <div className="window-size-presets">{[0.75, 1, 1.5, 2, 3].map(scale => <button key={scale} type="button" aria-label={`창 크기 ${scale * 100}%`} aria-pressed={Boolean(state && Math.abs(state.scale - scale) < 0.01)} onClick={() => void window.mate?.setWindowScale(scale).then(setState)}>{scale * 100}%</button>)}</div>
