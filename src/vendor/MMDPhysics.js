// Adapted from three.js r169 (MIT; public/physics/THREE-LICENSE.txt).
// Mate adds deterministic resource disposal and frees construction temporaries.
import {
	Bone,
	BoxGeometry,
	CapsuleGeometry,
...
class MMDPhysics {
	...
	setGravity( gravity ) {
		...
		const value = new Ammo.btVector3( gravity.x, gravity.y, gravity.z );
	...
	dispose() {
		...
		for ( const entry of this.bodies ) {
			...
			Ammo.destroy( entry.boneOffsetForm );
			Ammo.destroy( entry.boneOffsetFormInverse );
		}
		Ammo.destroy( this.world );
		for ( const value of this.worldResources || [] ) Ammo.destroy( value );
		for ( const pool of [ this.manager.transforms, this.manager.quaternions, this.manager.vector3s ] ) {
			for ( const value of pool ) Ammo.destroy( value );
			pool.length = 0;
		}
		this.bodies.length = 0; this.constraints.length = 0; this.world = null;
	...
	_createWorld() {
		...
		this.worldResources = [ solver, cache, dispatcher, config ];
		return world;
	...
	_init() {
		...
		function generateShape( p ) {
			...
			switch ( p.shapeType ) {
				...
				case 1:
				{
					const size = new Ammo.btVector3( p.width, p.height, p.depth );
